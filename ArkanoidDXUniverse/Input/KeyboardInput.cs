...
namespace ArkanoidDXUniverse.Input
{
    public class KeyboardInput
    {
        ...
        public void Draw(SpriteBatch batch)
        {
            ...
            else if (_isOskCaps)
            {
                ...
                var r = new Rectangle((int) (_oskOffset.X + _oskCapsRect.X), (int) (_oskOffset.Y + _oskCapsRect.Y),
                    _oskCapsRect.Width,
                    _oskCapsRect.Height);
                batch.Draw(Arkanoid.Pixel, r, null, Branding.ForegroundColor*.5f);
            }
            else
            {
            ...
            foreach (var b in _oskButtonHits)
            {
                var r = new Rectangle((int) (_oskOffset.X + b.Rect.X), (int) (_oskOffset.Y + b.Rect.Y), b.Rect.Width,
                    b.Rect.Height);
                batch.Draw(Arkanoid.Pixel, r, null, Branding.ForegroundColor*b.TimeToLive*.5f);
                // batch.Draw(Arkanoid.Pixel, new Rectangle(r.X,r.Y,r.Width,1), null, Color.Blue);
                // batch.Draw(Arkanoid.Pixel, new Rectangle(r.X, r.Y+r.Height, r.Width, 1), null, Color.Blue);
                // batch.Draw(Arkanoid.Pixel, new Rectangle(r.X, r.Y, 1, r.Height), null, Color.Blue);
                // batch.Draw(Arkanoid.Pixel, new Rectangle(r.X+r.Width, r.Y, 1, r.Height), null, Color.Blue);
            }
            batch.End();
        }
        ...
        public IEnumerable<Keys> TypedKeys => Enum.GetValues(typeof (Keys)).Cast<Keys>().Where(k => TypedKey(k));
