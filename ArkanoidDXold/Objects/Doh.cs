...
namespace ArkanoidDX.Objects
{
    ...
    public enum DohState { OpenMouth, CloseMouth }
    ...
    public class Doh : GameObject
    {
        ...
        public override sealed Sprite Texture
        {
            get { return Sprites.EnmDoh; }
        }

        public Sprite Border
        {
            get { return Sprites.CmnDohBoarder; }
        }

        public Sprite Web
        {
            get { return Sprites.EnmDohWeb; }
        }
        ...
        public Doh(ArkanoidDX game, PlayArena playArena) : base(game)
        {
            PlayArena = playArena;
            DohBdrRect = new Rectangle((int)(PlayArena.Bounds.Right - (Sprites.CmnDohBoarder.Width + Sprites.BgDoh.Width)), PlayArena.Bounds.Top,
                                     (int)Sprites.CmnDohBoarder.Width, (int)Sprites.CmnDohBoarder.Height);
            DohRect = new Rectangle((int)(DohBdrRect.Center.X - Sprites.EnmDoh.Center.X), (int)(Sprites.BgDoh.Height + PlayArena.Bounds.Top),
            ...
            if (IsAlive)
            {
                ...
                foreach (var b in PlayArena.Balls)
                {
                    ...
                    if(Collisions.IsCollision(b,DohRect,out d,out c))
                    {
                        Life--;
                        b.Deflect(c, d);
                        Game.Audio.Play(Sounds.VausEnter);
                        Hit = 1f;
                        if (Life == 0)
