...
namespace ArkanoidDX.Input
{
    public class KeyboardInput
    {
        ...
        public string CurrentChar
        {
            get
            {
                if (_currentString == string.Empty) return string.Empty;
                return _currentString.ToCharArray()[_currentString.Length - 1].ToString();
            }
        }
        ...
        public void Update(GameTime gameTime, ArkanoidDX engine)
        {
            _prevKey = _currKey;
            _currKey = Keyboard.GetState();

            if (_prevKey != _currKey)
            {
                foreach (Keys t in _keys)
                {
